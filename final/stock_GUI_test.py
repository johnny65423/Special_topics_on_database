...
class Stock():
    def __init__(self) -> None:
        ...
        self.stock_record = pd.DataFrame( { "stock_code":[], "date":[], "tradingVolume":[], "trading":[] } )
    ...
    def buy_and_sell_stock( self, stock_code, date ):
      """
      string stock_code
      string date
      ? curosr
      ...
      cursor = self.connect_SQL_server().cursor()
      if ( date.split("-")[2] == "10" ): # every month 10 add money
      ...
      self.connect_SQL_server().close()
      print(buy_df)
      for i in range( len(buy_df) ):
        stock_code_temp = str( buy_df['stock_code'][i] )
        print(stock_code_temp)
        print(self.current_money)
        print(date)
        if ( buy_df["buy_or_sell"][i] == 1 ):
          self.current_money = self.current_money-buy_df['today_c'][i]
          self.stock_current[stock_code_temp][0] = self.stock_current[stock_code_temp][0]+1
          self.stock_current[stock_code_temp][1] = self.stock_current[stock_code_temp][1]- buy_df['today_c'][i]
          # update record
        ...
        # end if
        elif ( buy_df["buy_or_sell"][i] == -1 ): # sell all
          self.current_money = self.current_money+buy_df['today_c'][i]*self.stock_current[stock_code_temp][0]
          self.stock_current[stock_code_temp][1] = self.stock_current[stock_code_temp][1]+ buy_df['today_c'][i]*self.stock_current[stock_code_temp][0]
      ...
      # end if
        ...
        return self.stock_record[self.stock_record["stock_code"].str.contains(stock_code)]
      # end else
    ...
    # 回傳交易紀錄
    # tochange: 串接交易紀錄code
    def getDealRecord( self, date, stockCode ):
        # 轉格式datetime->str
        ...
        # tochange: personal sql name
        if ( stockCode == "all" ):
            query = f"SELECT * FROM price_history where date < '{date}'"
        else:
            query = f"""SELECT * FROM price_history where  date < '{date}' and stock_code ='{stockCode}'"""

        df = pd.read_sql_query(query, conn)
        conn.close()
        
        df = self.buy_and_sell_stock( stockCode, date )
...
class Ui_MainWindow(object):
    ...
    def onStockChoiceChanged(self, index):
        ...
        # 交易紀錄更新
        selected_stock = self.stockChoice.currentText()
        df = self.stock.getDealRecord( self.currentDate, selected_stock )
